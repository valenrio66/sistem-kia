<?= $this->include('content/sidebar') ?>
<?= $this->include('content/header') ?>

<main class="content">
	<div class="container-fluid p-0">
		<h5 class="right-aligned" style="float: right">
			<a href="#">Home</a> / <a href="#">Kematian Maternal</a> / Data Kematian Maternal
		</h5>
		<h1 class="h3 mb-3"><b>Data Kematian Maternal</b></h1>
		<div class="col-md-6 mb-3">
			<div class="d-flex justify-content-start">
				<a href="<?= base_url('/dashboard/kematian_maternal/add') ?>" type="button" class="btn btn-info me-2">
					<i class="align-middle" data-feather="plus-circle"></i>
					Tambah Data Kematian Maternal
				</a>
				<a href="<?= base_url('/dashboard/kematian_maternal/export') ?>" type="button" class="btn btn-success me-2">
					<i class="align-middle" data-feather="file-text"></i>
					Generate Laporan
				</a>
			</div>
		</div>
		<div class="row justify-content-center">
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<div class="row no-gutters" style="height: 40px">
							<h1 class="h3 mt-2">Data Kematian Maternal</h1>
						</div>

						<hr />
						<div class="table-container">
							<table id="example" class="table table-striped" style="width: 100%">
								<thead>
									<tr style="text-align: center; vertical-align: middle">
										<th hidden></th>
										<th>Nomor RM</th>
										<th>Nama Lengkap Pasien</th>
										<th>Nama Suami</th>
										<th>Alamat</th>
										<th>Catat Buku KIA</th>
										<th>Tanggal Kematian</th>
										<th>Jam Kematian</th>
                                        <th>Penyebab Kematian</th>
                                        <th>Masa Kematian</th>
										<th>Action</th>
									</tr>
								</thead>
								<!-- Fetch Data Start -->
								<tbody>
									<?php foreach ($kematian_maternals as $km) : ?>
										<tr>
											<td hidden></td>
											<td>
												<?= $km['no_rm'] ?>
											</td>
											<td>
												<?= $km['nama_pasien'] ?>
											</td>
											<td>
												<?= $km['nama_suami'] ?>
											</td>
											<td>
												<?= $km['alamat'] ?>
											</td>
											<td>
												<?= $km['catat_buku_kia'] ?>
											</td>
											<td>
												<?= $km['tanggal_kematian'] ?>
                                            </td>
                                            <td>
												<?= $km['jam_kematian'] ?>
                                            </td>
                                            <td>
												<?= $km['penyebab_kematian'] ?>
                                            </td>
                                            <td>
												<?= $km['masa_kematian'] ?>
                                            </td>
											<td>
												<!-- Tambahkan tombol atau link aksi sesuai kebutuhan -->
												<!-- Contoh: -->
												<a href="<?= base_url('/dashboard/kematian_maternal/update/' . $pn['id_kematian_maternal']) ?>" class="btn btn-info">Edit</a>
												<a href="<?= base_url('kematian_maternal/delete/' . $pn['id_kematian_maternal']) ?>" onclick="return confirmDelete(event)" class="btn btn-danger">Delete</a>
											</td>
										</tr>
									<?php endforeach; ?>
								</tbody>
								<!-- Fetch Data End -->

							</table>
						</div>
						<div class="row mt-3">
							<div class="col-md-6">
								<div class="pagination">
									<button id="prevPageBtn" class="btn btn-primary">
										Previous
									</button>
									<span id="currentPage" class="mx-2">Halaman 1</span>
									<button id="nextPageBtn" class="btn btn-primary">
										Next
									</button>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</main>

<?= $this->include('content/footer') ?>
<script src="<?= base_url('js/kematian_maternal/delete.js') ?>"></script>